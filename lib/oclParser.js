...
import IsNotEmptyExpression from './../lib/expressions/isEmptyExpression';
...
const ImpliesExpressionPattern = `(${FunctionCallExpressionPattern}|${OperationCallExpressionPattern})\\simplies\\s(${FunctionCallExpressionPattern}|${OperationCallExpressionPattern})`;
...
class OclParser {
    ...
    parsePartialOcl(oclExpression) {
        ...
        if (match = OclParser.isExpression(IteratorExpressionPattern, oclExpression)) {
            ...
            expr = new ImpliesExpression(this.parsePartialOcl(match[1]), this.parsePartialOcl(match[22]))
        }
            ...
            expr = new BooleanExpression(match[1]);
        }
            ...
            expr = new StringExpression(match[1]);
        }
    ...
    parseOclFunctionCall(oclExpression, match) {
        let functionName = match[4].toUpperCase();

        if(functionName === 'ISEMPTY') {
            return new IsEmptyExpression(this.parsePartialOcl(match[1]));
        }
        else if(functionName === 'ISNOTEMPTY') {
            return new IsNotEmptyExpression(this.parsePartialOcl(match[1]));
        }
