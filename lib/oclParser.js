...
const InvariantExpressionPattern = `(inv|invariant):\\s(.*)`;
...
export default class OclParser {
    constructor(oclExpression) {
        this.oclExpression = OclParser.trimLeft(oclExpression);
    }
    ...
    parseInvariant(oclExpression) {
        ...
        if (match = OclParser.isExpression(InvariantExpressionPattern, oclExpression)) {
            match[2] = match[2] ? match[2].trim() : match[2];
            if(match[2]) {
                return match[2] ? new InvariantExpression(this.parsePartialOcl(match[2])) : {};
            }
    ...
    parsePartialOcl(oclExpression) {
        ...
        if (match = OclParser.isExpression(InvariantExpressionPattern, oclExpression)) {
            expr = {
                type: 'InvariantExp',
                expr: match[2] ? this.parsePartialOcl(match[2]) : {}
            }
        }
        else if (match = OclParser.isExpression(IteratorExpressionPattern, oclExpression)) {
            const iterators = [];
    ...
    static isExpression(regex, expression) {
        return new RegExp(`^${regex}`).exec(expression);
    }
}
