...
export class OclRuleVisitor {
    constructor(obj) {
        ...
        this.evaluatedContexts = 0;
        this.labelsToExecute = [];
    }

    setObjectToEvaluate(obj) {
        this.obj = obj;
    ...
    visitPackageDeclaration(expr) {
        let contextsToVisit = expr.contexts
    ...
    visitOclIsKindOfExpression(expr) {
        let source = expr.source.visit(this);
        let body = expr.body ? expr.body.visit(this) : undefined;
        ...
        if (collection instanceof Array) {
            if (!expr.iterators || expr.iterators.length === 0) {
                ...
                return !collection.some(c => {
                    ...
                    expr.body.variables[expr.iterators[0]] = undefined;
                    return result;
                });
                ...
                for (let i = 0; i < sourceLength; i++) {
                    ...
                    for (let j = i + 1; j < sourceLength; j++) {
                        ...
                        expr.body.variables[expr.iterators[1]] = undefined;
    ...
    visitVariableExpression(expr) {
        ...
        if (parts[0] === 'self') {
            ...
            let type = this.registeredTypes[expr.source]
            if (type) {
                return type
            } else {
                o = this.obj;
    ...
    visitSqrtExpression(expr) {
        let sqrt = expr.body ? expr.body.visit(this) : 2;
